...
{
	return request_irq(imxtm->irq, mxc_timer_interrupt,
			   IRQF_TIMER | IRQF_IRQPOLL /*| IRQF_TRIGGER_RISING | IRQF_EARLY_RESUME*/, "i.MX Timer Tick", imxtm);
}
#endif
...
static int __init _mxc_timer_init(struct imx_timer *imxtm)
{
    ...
    writel_relaxed(tcn + 24000000, imxtm->base + imxtm->gpt->reg_tcmp);
...
static int __init rfnm_gpt_init(void)
{
    ...
    np = of_find_node_by_path("/rfnm_gpt");        
	...
	gpt_iomem = ioremap(0x302d0000, SZ_4K);
...
MODULE_PARM_DESC(device, "RFNM GPT Driver");
